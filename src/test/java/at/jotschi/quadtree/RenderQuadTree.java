...
public class RenderQuadTree extends JPanel implements KeyListener,
		MouseListener {

	private QuadTree<String> tree;
	Vector<NodeElement<String>> selectedElements = new Vector<NodeElement<String>>();

	private static Logger log = Logger.getLogger(RenderQuadTree.class);
	...
	private void drawCells(Node<String> node, Graphics g) {
		...
		Vector<NodeElement<String>> elements = node.getElements();
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getButton() == 1) {
			...
			selectedElements = tree.getElements(p);
		}
		repaint();
	}
